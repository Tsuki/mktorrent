...
class Torrent
  ...
  attr_accessor :info, :filehashes, :piecelength, :files, :defaultdir, :tracker, :size, :privacy, :webseed, :tracker_list,:length,:path
  ...
  def add_file(filepath)
    ...
    if File.exist?(filepath)
      ...
      @path = File.basename(path_for_torrent[0])
      @length = File::open(filepath, "rb").size
  ...
  private
    ...
    def path_for_torrent_from_file(filepath)
      unless @dirbase.empty?
        filepath = filepath..force_encoding('ASCII-8BIT').sub(@dirbase, '')
      end
